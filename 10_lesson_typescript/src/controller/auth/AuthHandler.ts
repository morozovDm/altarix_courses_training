...
const logout = (req: express.Request, res: express.Response) => res.status(200).send({ auth: false, token: null })
...
const register = (req: express.Request, res: express.Response) => {
  ...
  UserModel.findOne({ username })
    ...
    .then((user : IUser) => {
      ...
      new UserModel({ username, password }).save().then(() => res.status(200).send("item saved to database"))
        .catch((err: Error) => res.status(400).send(`unable to save to database ${err}`));
    })
